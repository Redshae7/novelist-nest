...
| **Price** | $24.99 one-time (impulse territory vs. competitors' $60+), includes **two versions** (Suno returns 2 tracks per render — zero extra cost, doubled perceived value). One optional Deluxe bump (+$9.99). |
...
2. Payment (webhook **and** redirect-verify, whichever lands first) flips
   `paid=true` and starts the Suno render.
...
netlify/lib/core.mjs   shared: blobs, rate limits, Suno start/refresh, gating
netlify/functions/song.mjs             /api/song (lyrics/revise/edit/status/gift/lead/track/callback)
...
| `SUNO_API_KEY` | ✅ for audio | — | Music. Without it, paid songs queue as `no_provider`. |
| `SUNO_API_BASE` | | `https://api.sunoapi.org` | |
| `SUNO_MODEL` | | `V4_5` | |
| `STRIPE_SECRET_KEY` | ✅ | — | |
| `STRIPE_WEBHOOK_SECRET` | ✅ | — | `whsec_…` |
| `SITE_URL` | recommended | request origin | Used for Suno callbacks + webhook-side generation. |
| `CURRENCY` | | `usd` | |
...
4. Health check: `GET /api/song` returns `{status, suno, anthropic}`.
